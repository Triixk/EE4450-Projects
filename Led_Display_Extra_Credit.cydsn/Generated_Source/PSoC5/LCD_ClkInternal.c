...
/*******************************************************************************
* Function Name: LCD_ClkInternal_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_Start(void) 
{
    /* Set the bit to enable the clock. */
    LCD_ClkInternal_CLKEN |= LCD_ClkInternal_CLKEN_MASK;
	LCD_ClkInternal_CLKSTBY |= LCD_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LCD_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    LCD_ClkInternal_CLKEN &= (uint8)(~LCD_ClkInternal_CLKEN_MASK);
	LCD_ClkInternal_CLKSTBY &= (uint8)(~LCD_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LCD_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_StopBlock(void) 
{
    if ((LCD_ClkInternal_CLKEN & LCD_ClkInternal_CLKEN_MASK) != 0u)
    {
...
#if defined(LCD_ClkInternal__CFG3)
        CLK_DIST_AMASK = LCD_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LCD_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LCD_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LCD_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LCD_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LCD_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        LCD_ClkInternal_CLKSTBY &= (uint8)(~LCD_ClkInternal_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: LCD_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = LCD_ClkInternal_GetSourceRegister();
    uint16 oldDivider = LCD_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LCD_ClkInternal_CLKEN & LCD_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LCD_ClkInternal_DIV_PTR, clkDivider);
                LCD_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LCD_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LCD_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LCD_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: LCD_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 LCD_ClkInternal_GetDividerRegister(void) 
{
    return CY_GET_REG16(LCD_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: LCD_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_SetModeRegister(uint8 modeBitMask) 
{
    LCD_ClkInternal_MOD_SRC |= modeBitMask & (uint8)LCD_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: LCD_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_ClearModeRegister(uint8 modeBitMask) 
{
    LCD_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LCD_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: LCD_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LCD_ClkInternal_GetModeRegister(void) 
{
    return LCD_ClkInternal_MOD_SRC & (uint8)(LCD_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: LCD_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = LCD_ClkInternal_GetDividerRegister();
    uint8 oldSrc = LCD_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LCD_ClkInternal_MOD_SRC |= CYCLK_SSS;
        LCD_ClkInternal_MOD_SRC =
            (LCD_ClkInternal_MOD_SRC & (uint8)(~LCD_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LCD_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LCD_ClkInternal_GetSourceRegister(void) 
{
    return LCD_ClkInternal_MOD_SRC & LCD_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LCD_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void LCD_ClkInternal_SetPhaseRegister(uint8 clkPhase) 
{
    LCD_ClkInternal_PHASE = clkPhase & LCD_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LCD_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 LCD_ClkInternal_GetPhaseRegister(void) 
{
    return LCD_ClkInternal_PHASE & LCD_ClkInternal_PHASE_MASK;
}
